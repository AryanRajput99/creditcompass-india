...
import { ExternalLink, ChevronRight, RotateCcw, BadgeIndianRupee, CheckCircle } from 'lucide-react';
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="text-center mb-10">
          ...
          <button
            ...
            className="mt-3 text-xs font-bold text-blue-400 hover:text-blue-300 transition-colors underline"
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {step.options.map((option) => (
                <button
                  ...
                  className="group flex items-center gap-4 p-4 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 hover:border-blue-400/50 transition-all text-left"
                >
                  ...
                  <ChevronRight className="w-4 h-4 text-slate-500 ml-auto flex-shrink-0 group-hover:text-blue-400 transition-colors" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-4 mb-8">
              {recommendations.map((offer, idx) => (
                <div
                  key={offer.card_slug}
                  className={`bg-white/5 border rounded-2xl p-6 ${idx === 0 ? 'border-emerald-500/50 ring-1 ring-emerald-500/20' : 'border-white/10'}`}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">#{idx + 1} Match</span>
                        {idx === 0 && (
                          <span className="text-[9px] font-extrabold bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-2 py-0.5 rounded-full uppercase tracking-widest">
                            Best Pick
                          </span>
                        )}
                      </div>
                      <p className="font-black text-white text-lg leading-tight">{offer.card_name}</p>
                      <p className="text-sm text-slate-400 font-medium">{offer.bank_name}</p>
                      <div className="flex items-center gap-1.5 mt-2">
                        <BadgeIndianRupee className="w-3.5 h-3.5 text-emerald-400" />
                    ...
                    <a
                      ...
                      className="flex-shrink-0 flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold text-sm transition-all shadow-lg shadow-emerald-600/20"
                    >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button
                onClick={reset}
                className="flex items-center justify-center gap-2 px-6 py-3 bg-white/10 border border-white/20 hover:bg-white/20 text-white rounded-xl font-bold text-sm transition-all"
              >
              ...
              <Link
                ...
                className="flex items-center justify-center gap-2 px-6 py-3 bg-white/5 border border-white/10 hover:bg-white/10 text-slate-300 hover:text-white rounded-xl font-bold text-sm transition-all"
              >
